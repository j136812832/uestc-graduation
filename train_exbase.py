...
from effib3_cc1_newarm_newffm import ZyjNet
from ex_base import BaseNet
...
def train(args, trainloader, model, criterion, optimizer, epoch, logger):
    ...
    if epoch == 499:
        ...
        plt.ylabel("loss")
        plt.plot(listepoch, listLoss)
        plt.savefig("./pic/loss/epoch499.jpg")
...
def main(args, logger):
    ...
    # print("the number of vgg_parameters: " + str(vgg_paramters))
    ...
    while flag == True:
        for epoch in range(start_epoch, args.max_epochs):
            ...
            if epoch % 1 == 0 or (epoch + 1) == args.max_epochs:
                ...
                print("test time is {}  ".format(testTime))
                ...
                if epoch % 5 == 0:
                    ...
                    plt.savefig("./pic/miou/miou.jpg")

                    plt.clf()
                    plt.xlabel("epoch")
                    plt.ylabel("oa")
                    plt.plot(listEpoche, listOa)
                    plt.savefig("./pic/oa/oa.jpg")

                    plt.clf()
                    plt.xlabel("epoch")
                    plt.ylabel("f1")
                    plt.plot(listEpoche, listF1)
                    plt.savefig("./pic/f1/f1.jpg")

                    plt.clf()
                    plt.xlabel("epoch")
                    plt.ylabel("acc")
                    plt.plot(listEpoche, listAcc)
                    plt.savefig("./pic/acc/acc.jpg")
                ...
                if epoch == 499:
                    ...
                    plt.ylabel("miou")
                    plt.plot(listEpoche, listMiou)
                    plt.savefig("./pic/miou/epoch499.jpg")

                    plt.clf()
                    plt.xlabel("epoch")
                    plt.ylabel("oa")
                    plt.plot(listEpoche, listOa)
                    plt.savefig("./pic/oa/epoch499.jpg")

                    plt.clf()
                    plt.xlabel("epoch")
                    plt.ylabel("f1")
                    plt.plot(listEpoche, listF1)
                    plt.savefig("./pic/f1/epoch499.jpg")

                    plt.clf()
                    plt.xlabel("epoch")
                    plt.ylabel("acc")
                    plt.plot(listEpoche, listAcc)
                    plt.savefig("./pic/acc/epoch499.jpg")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root', default='../../image/', help='data directory') #/media/ssd/zyj_data/vaismall/

    parser.add_argument('--batch_size', default=8, type=int)
    parser.add_argument('--max_epochs', type=int, default=600, help='the number of epochs: default 100 ')
    ...
    parser.add_argument('--savedir', default="./exbase/", help="directory to save the model snapshot")
    # parser.add_argument('--logFile', default= "log.txt", help = "storing the training and validation logs")
    parser.add_argument('--gpu', type=str, default="2", help="default GPU devices (1)")
